...
class FavoritesLogicTest(TestCase):
    ...
    def test_toggle_prevents_duplicate_favorites(self):
        ...
        self.client.post(url)
        self.assertEqual(UserFavouriteArticle.objects.filter(user=self.user, article=self.article).count(), 0)
    ...
    def test_database_integrity_unique_constraint(self):
        ...
        count = UserFavouriteArticle.objects.filter(user=self.user, article=self.article).count()
        self.assertLessEqual(count, 1, "The database contains duplicate favorites for the same user and article!")

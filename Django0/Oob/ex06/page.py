...
class Page:
    ...
    def __recursive_check(self, node) -> bool:
        ...
        if isinstance(node, Html):
            ...
            if len(node.content) == 0 or not all(isinstance(c, Li) for c in node.content):
                return False
            ...
            if (has_th and has_td) or not all(isinstance(c, (Th, Td)) for c in node.content):
                return False
...
def main():
    ...
    print(f"Is valid: {valid_p.is_valid()}")
    valid_p.write_to_file("index.html")

    print("\n--- Test 2: Invalid Tr (Mixed Th and Td) ---")
    invalid_tr = Page(Table([Tr([Th(Text("H")), Td(Text("D"))])]))
    print(f"Is valid: {invalid_tr.is_valid()}")

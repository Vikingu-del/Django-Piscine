...
def test_elem_basics():
    ...
    # Argument names :
    assert (
        str(Elem(tag="body", attr={}, content=Elem(), tag_type="double"))
        == "<body>\n  <div></div>\n</body>"
    )
    ...
    assert (
        str(Elem(content=[Text("foo"), Text("bar"), Elem()]))
        == "<div>\n  foo\n  bar\n \
 <div></div>\n</div>"
    )
...
def test_empty_texts():
    ...
    assert (
        str(Elem(content=[Text("foo"), Text(""), Elem()]))
        == "<div>\n  foo\
\n  <div></div>\n</div>"
    )
...
def test_errors():
    ...
    try:
        ...
        assert type(e) is Elem.ValidationError
    # The right way :
    ...
    # The right way :
    assert (
        str(Elem(content=[Text("foo"), Elem(), Text(1)]))
        == "<div>\n  foo\n  <div></div>\n  1\n</div>"
    )
...
def test_embedding():
    assert (
        str(Elem(content=Elem(content=Elem(content=Elem()))))
        == """<div>
  <div>
    <div>
      <div></div>
